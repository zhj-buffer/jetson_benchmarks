...
class utilities():
    def __init__(self, jetson_devkit, gpu_freq, dla_freq, log):
    ...
    def set_power_mode(self, power_mode, jetson_devkit):
        ...
        subprocess.call('sudo {} {}'.format(power_cmd0, power_cmd1), shell=True,
                        stdout=FNULL)
        self.log.logger.info('Setting Jetson {} in max performance mode'.format(jetson_devkit))

    def set_jetson_clocks(self):
        clocks_cmd = 'jetson_clocks'
        subprocess.call('sudo {}'.format(clocks_cmd), shell=True,
                        stdout=FNULL)
        self.log.logger.info("Jetson clocks are Set")
    ...
    def set_user_clock(self, device):
        #self.log.logger.info(self.jetson_devkit)
        if self.jetson_devkit == 'tx2':
    ...
    def set_clocks_withDVFS(self, frequency, device):
        ...
        self.log.logger.info('{} frequency is set from {} Hz --> to {} Hz'.format(device, from_freq, to_freq))
    ...
    def check_trt(self):
        if not os.path.isfile('/usr/src/tensorrt/bin/trtexec'):  # Check if TensorRT is installed
            self.log.loggger.info("Exiting. Check if TensorRT is installed \n Use ``dpkg -l | grep nvinfer`` ")
            return True
        return False
