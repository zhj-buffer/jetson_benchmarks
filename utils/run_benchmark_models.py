from utils import load_store_engine, read_write_data, utilities, logger
import time
import subprocess

class run_benchmark_models():
    def __init__(self, csv_file_path, model_path, precision, benchmark_data, log):
        self.log = log
    ...
    def execute(self, read_index):
        ...
        self.log.logger.info('------------Executing {}------------\n'.format(self.model_name))
        ...
        self.trt_engine = load_store_engine(model_path=self.model_path, model_name=model_ext, num_devices=num_devices,
                                            ...
                                            model_input=model_input, model_output=model_output, log=self.log)
    ...
    def report(self):
        ...
        self.log.logger.info('Model Name: {} GPU: {}, DLA: {} \nFPS:{:.2f} \n'.format(self.model_name, self.batch_size_gpu, self.batch_size_dla, latency_fps[3]))
        self.log.logger.info('--------------------------\n')
    ...
    def remove(self):
        ...
        self.log.logger.info('Wall Time for running model (secs): {}\n'.format(self.wall_time))
